...
class Curriculum:
    def set_to(self, low, high, value=0.01):
        # print("low,high",low," ",high," ",high,self.grid >= low[:, None],self.grid <= high[:, None])
        inds = np.logical_and(
            self.grid >= low[:, None],
            self.grid <= high[:, None]
        ).all(axis=0)
    ...
    def sample_uniform_from_cell(self, centroids):
        ...
        # print("low:",low,"high:",high,"centroids",centroids)
...
class RewardThresholdCurriculum(Curriculum):
    ...
    def get_local_bins(self, bin_inds, ranges=0.5):
        if isinstance(ranges, float):
            ranges = np.ones(self.grid.shape[0]) * ranges
        bin_inds = bin_inds.reshape(-1)
        # print("self.grid",self.grid)

        # print("self.grid[:, bin_inds, None]",self.grid[:, bin_inds, None])

        # print("ranges",self.grid[:, bin_inds, None] - ranges.reshape(-1, 1, 1))
        #  [12  8  8  6]
        adjacent_inds = np.logical_and(
            self.grid[:, None, :].repeat(bin_inds.shape[0], axis=1) >= self.grid[:, bin_inds, None] - ranges.reshape(-1, 1, 1),
            self.grid[:, None, :].repeat(bin_inds.shape[0], axis=1) <= self.grid[:, bin_inds, None] + ranges.reshape(-1, 1, 1)
        ).all(axis=0)
        # print("adjacent_inds",adjacent_inds)
        return adjacent_inds

    def update(self, bin_inds, task_rewards, success_thresholds, local_range=0.5, weights_up=1e-6):
        ...
        # print("bin_inds[is_success]",bin_inds[is_success])
